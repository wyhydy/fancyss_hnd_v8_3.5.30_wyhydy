...
- 路由器显示版本：`3.5.30.modify:wyhydy.0.3`。
- 插件完整重启成功，代理出口检测成功。
- AnyTLS PID `24034`、ipt2socks PID `24029`。
- 关闭执行重启的 SSH 会话后重新连接，两个 PID 均保持不变，父进程均为 PID 1。
- `127.0.0.1:23456` 和 `0.0.0.0:3333` 保持监听。
- `rules_healthcheck proxy` 返回 `0`。
- 服务器与安装包中的 `ss_base.sh`、`ssconfig.sh` MD5 完全一致。
- `ss_update.sh` 未修改，MD5 为 `c17f6db04f199be1bb19f1171b633e97`。
